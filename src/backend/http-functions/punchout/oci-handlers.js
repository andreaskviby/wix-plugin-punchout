...
async function handleOciStart(request, method) {
  try {
    ...
    await logTransaction('in', 'oci', null, '/punchout/oci/start', 200, JSON.stringify(params), request.headers);
    ...
    await logTransaction('out', 'oci', buyer.buyerId, '/punchout/oci/start', 302, `Redirect to: ${storefrontUrl}`);
...
export async function post_oci_return(request) {
  try {
    ...
    // Log transaction
    await logTransaction('out', 'oci', session.buyerId, '/punchout/oci/return', response.status, JSON.stringify(ociFields));
